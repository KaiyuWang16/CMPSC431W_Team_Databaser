 ...
 if ($_GET['sub']!=""){
  ...
  ?>
   <script>
    ...
    window.location = 'productsdisplay.php';
   </script>
<?php   
 }
?>
...
<div id="mainContent">
	...
	<form name="frm_main" id="frm_main" method="post" action="">
      <table border="1" cellpadding="0" cellspacing="0" width="1340" height="300">
	<!-- MSTableType="layout" -->
<tr>
...
<?php
	...
	$targetpage = "productsdisplay_cat.php"; 	//your file name  (the name of this file)
	...
	$query="SELECT COUNT(*) AS num FROM item i, sitem s, category c WHERE i.tid = s.tid AND c.caid = '$selected_caid' AND i.caid = c.caid";
	//$query="SELECT COUNT(*) AS num FROM sitem s, item i, category c WHERE i.tid = s.tid AND c.caName = '$selected_caid'";
	//echo $query;
	...
	$sitems_query = "SELECT * FROM item i, sitem s, category c WHERE i.tid = s.tid AND c.caid = '$selected_caid' AND i.caid = c.caid";
    ...
    if(isset($_POST['sub']))
    {
	  ...
	  for($b=0;$b<$num;$b++)
		...
		//	echo "$pid";
	     ...
	     echo "<script>window.location='productsdisplay_cat.php?un=$un&sub=$selected_caid'</script>";
	  }
