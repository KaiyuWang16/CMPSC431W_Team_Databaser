...
<?php
	...
	$targetpage = "productsdisplay_search.php"; 	//your file name  (the name of this file)
	...
	$query="SELECT COUNT(*) AS num FROM item, sitem WHERE item.tid = sitem.tid AND item.title LIKE '%$key%'";
	...
	$sitems_query = "SELECT * FROM item i, sitem s WHERE i.tid = s.tid AND i.title LIKE '%$key%'";
    ...
    if(isset($_POST['sub']))
    {
	  ...
	  for($b=0;$b<$num;$b++)
		...
		//	echo "$pid";
	     ...
	     echo "<script>window.location='productsdisplay_search.php?un=$un&key=$key'</script>";
	  }
