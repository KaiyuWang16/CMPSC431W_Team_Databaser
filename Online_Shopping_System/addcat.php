...
<!-- Code for inserting into data base -->
 <?php
  if(isset($_POST['submitMain']))
  {
   ...
   $get= @mysql_query("SELECT caid FROM category WHERE caName = '$subcat'")or die(mysql_error());
   ...
   $catID .=date("YmdHis");
   if($check==1){
   $query = mysql_query("INSERT INTO category (caid,caName,parentID) VALUES ('$catID','$cat','$row')")
    or die(mysql_error());
    echo "<script>alert('Category Added sucessfully !!');</script>";
  }
  else
   {
     echo '<div align="center"><strong><font color="#FF0000">Category does not exist !!</font></Strong></div>';
   }
}
